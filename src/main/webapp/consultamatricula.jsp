...
<%@page import="service.impl.AlumnoServiceImpl"%>
<%@page import="java.util.List"%>
<%@page import="model.CursoMatricula"%>
        ...
        <span class="navbar-toggler-icon"></span>
			...
			<main class="content px-3 py-2">
				...
				<div class="container-fluid matricula-container"
					id="matriculaAlumno">
					<div class="row">
						<h3 class="tittle-matricula">Consulta Matrícula</h3>
						<div class="col-md-4">
								...
								<form action="MatriculaServlet?type=registrar" method="post"
									...
									<div class="form-group matricula-group">
										<label class="label-matricula">Código de Alumno</label> <input
											...
											value="${alumn.idAlumno}" name="txtNumMatricula"
											id="txtNumMatricula">
										<br>
<a href = "#" onclick="obtenerAlumno();" type="button" class="btn btn-primary">Buscar</a>
									</div> 
									<br>
									<div class="form-group">
										<label class="label-matricula">Nombre de Alumno</label> <input
											class="form-control" type="label" value="${alumn.nomAlumno}"
											name="txtCodAlumno" id="txtCodAlumno">
									</div>
									<br>
									<div class="form-group">
										<label class="label-matricula">Apellido de Alumno</label> <input
											class="form-control" type="label" value="${alumn.apeAlumno}"
									...
									<table id="miTabla" class="table table-success table-striped">
										<thead>
											<tr>
												...
												<th class="centered">Nombre Curso</th>
												<th class="centered">Ciclo</th>
												<th class="centered">Credito Curso</th>
												<th class="centered">Horas Curso</th>
										</thead>
										<tbody>
											<%
											List<CursoMatricula> listCursoMatri = (List<CursoMatricula>) request.getAttribute("listMatri");
											if (listCursoMatri != null) {
												for (CursoMatricula c : listCursoMatri) {
											%>
											<tr>
												<td><%=c.getNumMatricula()%></td>
												<td><%=c.getNomCurso()%></td>
												<td><%=c.getCiclo()%></td>
												<td><%=c.getCreditCurso()%></td>
												<td><%=c.getHorasdeCurso()%></td>
...
function obtenerAlumno(){
	var url = "AlumnoServlet?type=getAlumno&idAlumno=" + $("#txtNumMatricula").val();
	location.href=url;
}
</script>
